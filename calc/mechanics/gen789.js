...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    ...
    if (field.defenderSide.isProtected && !breaksProtect && !move.usePlus) {
    ...
    if (gen.num === 10) {
        if (typeEffectiveness == 0.5) {
            typeEffectiveness = 0.6;
        }
        if (typeEffectiveness == 0.25) {
            typeEffectiveness = 0.3;
        }
    }
    ...
    if (fixedDamage) {
        if (attacker.hasAbility('Parental Bond')) {
            ...
            if (gen.num === 10) {
                result.damage = Math.floor((0, util_2.OF32)(result.damage * 0.7));
            }
        }
        return result;
    }
    var proportionalDamage = (0, util_2.handleProportionalDamageMoves)(defender, move);
    ...
    for (var i = 0; i < 16; i++) {
        ...
        if (gen.num === 10) {
            damage[i] = Math.floor((0, util_2.OF32)(damage[i] * 0.7));
            if (move.usePlus) {
                damage[i] = Math.floor((0, util_2.OF32)(damage[i] * (typeEffectiveness > 1 ? 1.3 : 1.2)));
                desc.isPlus = true;
            }
        }
    }
    ...
    if (move.timesUsed > 1 || move.hits > 1) {
        ...
        for (var times = 1; times < numAttacks; times++) {
            ...
            for (var i = 0; i < 16; i++) {
                ...
                if (gen.num === 10) {
                    newFinalDamage = Math.floor((0, util_2.OF32)(newFinalDamage * 0.7));
                    if (move.usePlus) {
                        newFinalDamage = Math.floor((0, util_2.OF32)(newFinalDamage * (typeEffectiveness > 1 ? 1.3 : 1.2)));
                        desc.isPlus = true;
                    }
                }
...
function calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc) {
    ...
    if (field.attackerSide.isCharged &&
        move.hasType('Electric')) {
        atMods.push(8192);
        desc.isCharged = true;
    }
